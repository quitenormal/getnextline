...
/*   Updated: 2022/06/23 20:16:37 by yjirapin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
is successful.
2) Starting out with bytes_read = 1 (just so that
...
if bytes_read is not 0 then we keep on going.
4) If there is no newline in mainman and the bytes_read isn't
...
6) After a successful read, the bytes_read is changed to the
...
or until bytes_read = 0 (which means that we're at the
end of the file).
...
char	*ft_read4myman(int fd, char *mainman)
{
	...
	if (!buff)
		return (NULL);
	bytes_read = 1;
	while (!check4nl(mainman) && bytes_read != 0)
	{
